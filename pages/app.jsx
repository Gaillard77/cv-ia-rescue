...
`.trim();
  ...
  async function exportCVPro() {
    ...
    const effectivePhoto =
      photoDataUrl && photoDataUrl.startsWith("data:image") ? photoDataUrl : photoUrl?.trim() ? photoUrl.trim() : "";
  ...
  return (
    <div className="min-h-screen text-white bg-gradient-to-b from-bg1 to-bg2">
      <div className="max-w-[1100px] w-[92vw] mx-auto py-8">
        ...
        {/* ---- ton interface existante inchangée ci-dessous ---- */}

        {/* --------- SECTION CV --------- */}
        {/* (tout le reste de ton code inchangé) */}
        {/* Upload / Offre / Formulaire / Génération / Exports / Sélecteurs / Erreurs / Aperçus */}
        {/* ----- contenu que tu avais déjà (copié tel quel) ----- */}
        ...
        {/* (… le reste de ta page identique, je laisse tel quel) */}
        {/* ---------- FORMULAIRE CV, LETTRE, BOUTONS, APERÇUS, ETC. ---------- */}
...
export async function getServerSideProps(ctx) {
  ...
  if (!session) {
    return {
      redirect: { destination: "/auth/signin?callbackUrl=/app", permanent: false },
    };
  }
  return { props: {} };
}
