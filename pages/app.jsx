...
`.trim();
  ...
  async function generateLetter(){
    ...
    try{
      ...
      const autoProfile = buildLetterProfileAuto(data, { cvName, cvTitle, cvEmail, cvPhone, cvLocation });
  ...
  return (
    <div className="min-h-screen text-white bg-gradient-to-b from-bg1 to-bg2">
      <div className="max-w-[1100px] w-[92vw] mx-auto py-6">
        {/* Header avec utilisateur + Déconnexion */}
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <span className="text-white/70 text-sm hidden sm:block">{user?.name || "Utilisateur"}</span>
            <button
              onClick={() => signOut({ callbackUrl: "/" })}
              className="rounded-lg px-3 py-1.5 border border-white/20 hover:border-white/50 text-sm"
            >
              Se déconnecter
            </button>
          </div>
        ...
        <div className="border border-white/10 rounded-2xl shadow-soft bg-gradient-to-b from-card1 to-card2 p-5 mt-6">
          ...
          <div className="mt-4 grid md:grid-cols-[1fr,auto] items-start gap-4">
            <div>
              ...
              />
              <div className="text-white/60 text-xs mt-1">Ou importez un fichier (JPG/PNG) — l’image sera intégrée au PDF.</div>
              <div className="mt-2">
            ...
            <div className="flex items-center justify-center">
              <div className="w-24 h-24 rounded-full overflow-hidden border border-white/20 bg-white/10">
                ...
                <img alt="aperçu photo" src={photoDataUrl || photoUrl || ""} className="w-full h-full object-cover"
          ...
          <div className="mt-4">
            ...
            <textarea className="w-full min-h-[120px] rounded-xl border border-white/15 bg-[#0f1526] text-white p-3"
                      value={cvSummary} onChange={(e)=>setCvSummary(e.target.value)} placeholder="Décrivez-vous..." />
          </div>
...
// ====== Protection serveur : redirection si pas connecté
